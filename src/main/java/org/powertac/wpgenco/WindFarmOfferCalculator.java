...
public class WindFarmOfferCalculator {
	...
	private Map<Timeslot, Integer> mapTimeslotToHour = new HashMap<Timeslot, Integer>();
	 ...
	 * @param ts timeslot for which the calculation is done.
	...
	private double determineOfferCapacity(Timeslot ts) {
		...
		double offerCap = 0;
		for (double currCap = 0; currCap <= maxCapacity; currCap += currCap * stepSize) {
	...
	private double getRevenue(double pbid, double mcp, double crd, double cru, Timeslot ts) {
		double mcpRevenue = pbid * mcp;
		//get imbalance revenue - positive revenue indicate profit, -ve revenue loss
	...
	private boolean getPrices(Timeslot ts, Double mcp, Double minp, Double maxp) {
		...
		if (mapTimeSlotClearingPrices.get(ts) == null) {
			...
			if (oldTs == null) {
				oldTs = timeslotRepo.makeTimeslot(new Instant(tsi)); //is this permitted?
			}
